...
```text
@bot	(x)comicn       	<+name: haizeiwang/海贼王> <+num: 话数> 搜索漫画话数
@bot	(y)comic        	<+name: haizeiwang/海贼王> 搜索漫画
...
@bot	(x)listtask     	任务列表
@bot	(x)help         	帮助信息
@bot	(y)抽奖           	<+bilibili-cookie> bilibili 抽奖, 自动转发up主的抽奖活动
@bot	(x)to           	查询车票信息, 只显示有票的班次, 例如: 'to 杭州东 绍兴北 20240216'
@bot	(x)t            	查询车票信息, 例如: 't 杭州东 绍兴北 20240216'
@bot	(y)ddys         	<+dy/dm>, 获取最新的电影、动漫资讯, 默认 +dy
@bot	(y)draw         	<+prompt>: 使用 ai 画图
@bot	(y)天气           	<+城市> 获取天气信息, 默认杭州
@bot	(y)微博           	获取热搜 top50
@bot	(y)知乎           	获取热搜 top50
@bot	(x)jin10        	金十数据今日大事件
@bot	(x)webot        	微信机器人扫码登录
@bot	(y)图片           	返回动漫图片~
@bot	(x)bitget       	获取当前开仓币
@bot	(x)geo          	geo 查询
@bot	(x)bg-money     	获取当前资金
@bot	(x)kfc          	KFC 骚话
@bot	(x)tip          	生活小窍门
@bot	(x)dx           	新债查询
@bot	(y)stock        	分析股票
